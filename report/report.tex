...
\cite{voyager} drives long-horizon tasks through tool calling. Those works
focus on generalization and exploration; the present work instead
emphasizes safety: the LLM only proposes calls to a fixed allowlist of
skills, and a validator with bounded recovery guards every execution before
the robot moves.
...
\end{tikzpicture}
\caption{System architecture: user request $\rightarrow$ planner $\rightarrow$
validator/executor $\rightarrow$ robot skills $\rightarrow$ simulation, with
structured results fed back for bounded recovery.}
\label{fig:arch}
\end{figure}
...
($\ge 0.35$\,m from all walls and furniture). The LLM used is DeepSeek
\texttt{deepseek-chat} \cite{deepseek} through the OpenAI-compatible API
\cite{openai-fc}.
...
Table~\ref{tab:results} summarizes the three required requests under both the
mock planner and the real LLM, plus the timeout-recovery scenario.
...
sessions and in 26.5\,s with the LLM. Every run converged within the stated
tolerances, with measured position errors in $0.084$--$0.099$\,m and heading
errors in $0.086$--$0.099$\,rad.
...
motion. The contribution is engineering value rather than algorithmic novelty:
combining function-calling LLMs with explicit skill contracts and bounded
recovery yields a system that is safe in practice and reproducible.
...
let the model repair its own plan. Future work includes occupancy-map based
planning, multi-request stateful dialogues, learned recovery policies, and
migration to a physical robot, since the executor already accepts any robot
driver that provides the three skill methods. As a safety consideration, API
credentials are supplied through environment variables only and are never
